...
- Run the project's unit tests before considering implementation complete. Follow the **make-commands** rule (`.kiro/steering/make-commands.md`) when running `make` targets — check local Go compatibility first, fall back to `skipper make` if incompatible, and never try to install or update the local toolchain.
